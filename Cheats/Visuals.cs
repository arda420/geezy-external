...
namespace elmencho.Cheats
{
    public class Visuals
    {
        ...
        private void _window_DrawGraphics(object sender, DrawGraphicsEventArgs e)
        {
            ...
            if (Properties.Settings.Default.watermark == true)
                DrawTextWithOutline("GEEZY | " + Properties.Settings.Default.text, 10, 5, Properties.Settings.Default.ftt, Properties.Settings.Default.watermarkColor, Properties.Settings.Default.watermarkColor2, Properties.Settings.Default.bold, Properties.Settings.Default.italic);
                    ...
                    if (Player.EntityBase != G.Engine.LocalPlayer.EntityBase)
                    {
                        ...
                        if (!Tools.IsNullVector2(Player2DPos) && !Tools.IsNullVector2(Player2DHeadPos) && Player.Valid)
                        {
                            ...
                            if (Player.IsTeammate)
                                drawcolor = Properties.Settings.Default.team;
                            else
                                drawcolor = Properties.Settings.Default.enemy;

                            #region Box
                            if (Properties.Settings.Default.boxtype == "Box")
                                DrawOutlineBox(Player2DPos.X - (BoxWidth / 2), Player2DHeadPos.Y, BoxWidth, BoxHeight, drawcolor);
                            else if (Properties.Settings.Default.boxtype == "Edge")
                                DrawBoxEdge(Player2DPos.X - (BoxWidth / 2), Player2DHeadPos.Y, BoxWidth, BoxHeight, drawcolor, 1);
                            else if (Properties.Settings.Default.boxtype == "Rounded Box")
                                DrawRoundedBox(Player2DPos.X - (BoxWidth / 2), Player2DHeadPos.Y, BoxWidth, BoxHeight, Properties.Settings.Default.radius, drawcolor);
                            if (Properties.Settings.Default.outline == true)
                                DrawFillOutlineBox(Player2DPos.X - (BoxWidth / 2), Player2DHeadPos.Y, BoxWidth, BoxHeight, drawcolor, Properties.Settings.Default.outlineColor);
                            #endregion
                            #region Health Bar
                            if (Properties.Settings.Default.healthbar == true)
                            {
                            ...
                            if (Properties.Settings.Default.snapline == true)
                                DrawLine(Main.MidScreen.X, Main.MidScreen.Y + Main.MidScreen.Y, Player2DPos.X, Player2DPos.Y, Properties.Settings.Default.snaplineColor);
